...
\begin{document}
    ...
    \subparagraph{\normalfont제출된 소스코드상에서 해당 함수는 1) \texttt{scanf}를 이용한 입력과 2) \texttt{_getch}를 이용한 두가지 동작을 제시한다. 각 동작의 실행 여부는 \texttt{App.h} 에 정의된 매크로인 \texttt{__PREFER_STANDARD}의 값에 따라 컴파일 타임에 결정된다.}

    \subparagraph{\normalfont전자의 경우 버퍼로 사용할 \texttt{char} 배열을 선언하고 }
