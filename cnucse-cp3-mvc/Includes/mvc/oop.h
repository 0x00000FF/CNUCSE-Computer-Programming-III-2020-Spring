...
#define STRUCT(class_name, members) typedef struct { members } class_name

#define METHOD_NAME(class_name, name) class_name##_##name
#define METHOD(class_name, return_type, name, ...) \
		return_type METHOD_NAME(class_name, name)(class_name* $this, __VA_ARGS__)

#define CTOR(class_name, ...) METHOD(class_name, void, CTOR, __VA_ARGS__)

#define INVOKE_METHOD(class_name, object, method_name, ...) class_name##_##method_name(object, __VA_ARGS__)

#define NEW(class_name, obj_name, ...) class_name* obj_name = (class_name*)malloc( sizeof(class_name) ); \
