...
linked_list_node* unsorted_linked_list_at (unsorted_linked_list* self, int order) {
    if (unsorted_linked_list_is_empty(self)) return NULL;
    
    int rev_pos            = self->size - order;
    linked_list_node* node = self->head;

    while(rev_pos-- && node != NULL) {
...
element           unsorted_linked_list_remove_head(unsorted_linked_list* self) {
    if (unsorted_linked_list_is_empty(self)) return -1;
    
    linked_list_node* prev_head = self->head;
    
    element value = linked_list_node_element(prev_head);
    self->head    = linked_list_node_next(prev_head);

    linked_list_node_delete(prev_head);
    return value;
}
